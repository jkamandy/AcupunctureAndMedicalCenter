...
const ServiceCards = ({ cardsInfoFirst, cardsInfoSecond }) => {
  ...
  return (
    <div ref={ref} className={`flex flex-col gap-y-6 w-[80%] justify-center items-center mt-12 transition-all duration-1000 ${
      ...
      <div className='flex justify-center w-screen mt-5'>
        ...
        {Object.entries(cardsInfoFirst).map(([title, description], index) => (
            <div><Card title={title} desc={description} bgImage={bgImages[index]}/></div>
        ))}
        </div>
        
        <div className='flex gap-6'>
        {Object.entries(cardsInfoSecond).map(([title, description], index) => (
            <div><Card title={title} desc={description} bgImage={bgImages[index+4]}/></div>
        ))}
        </div>
