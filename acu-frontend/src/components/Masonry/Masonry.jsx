...
const testimonials = [
  ...
  },
  {
    ...
    avatar: '/placeholder.svg',
    content:
      'Dr G informative, compassionate, knowledgeable and kind. I highly recommend. He has helped me with anxiety and hormonal issues tied to menopause.',
    rating: 5,
        ...
        <div className="mt-10 columns-1 gap-6 space-y-6 md:columns-2 lg:columns-3">
          {testimonials.map((t) => {
            ...
            return (
              <div
                key={t.id}
                className={`break-inside-avoid bg-gradient-to-br ${t.gradient} group rounded-xl border-0 p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl`}
              >
                <div className="mb-4 flex items-start justify-between">
                  <Quote className="h-8 w-8 text-gray-400 transition-colors group-hover:text-[#D6D6D6]" />
                  {/* <div className="flex gap-1">{renderStars(t.rating)}</div> */}
                </div>

                <div
                  className={`relative ${!isExpanded ? 'max-h-32 overflow-hidden' : ''} mb-6`}
                >
                  <blockquote className="font-spartan leading-relaxed text-gray-400 group-hover:text-[#D6D6D6]">
                    "{t.content}"
                  </blockquote>
                  {!isExpanded && (
                    <div className="pointer-events-none absolute bottom-0 left-0 h-10 w-full" />
                  )}
                </div>

                {t.content.length > 200 && (
                  <button
                    className="mb-4 font-spartan text-sm text-gray-400 hover:text-[#D6D6D6] hover:underline"
                ...
                /> */}
                  <div>
                    <div className="font-spartan font-semibold text-gray-500">
                      {t.name}
                    </div>
